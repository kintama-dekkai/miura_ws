...
class Curvature_local(Node):
   def __init__(self):
      ...
      self.k_off = 0.5
      self.k_smooth = 0.0
      self.turn_signal = String()
      self.turn_signal.data = 'straight'
      self.a = 0.7
   ...
   def local_plan_cb(self,msg:Path):
      ...
      if self.k_smooth > self.k_on:
         self.turn_signal.data = 'left'
      elif self.k_smooth < -self.k_on:
         self.turn_signal.data = 'right'
      elif self.turn_signal.data == 'left' and self.k_smooth < self.k_off:
         self.turn_signal.data = 'straight'
      elif self.turn_signal.data == 'right' and self.k_smooth > -self.k_off:
         self.turn_signal.data = 'straight'
      self.turn_signal_pub.publish(self.turn_signal)
      self.get_logger().info(f'{self.turn_signal.data}, {self.k_smooth}, {len(msg.poses)}')
